...
**Gate hack**: select a PPS of 12 or 24, enter gate types that are invalid at a chosen lower PPS (4 or 6), and then once entered, set the PPS to that lower value. The sequencer will still produce gate signals, but not exactly according to the pattern indicated. This can be explored and may actually be a way of producing other gate pattens than the ones provided by the sequencer.
